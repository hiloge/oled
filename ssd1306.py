...
class SSD1306:
  ...
  def rect(self, x, y, w, h, col):
    self.framebuf.rect(x, y, w, h, col) #col是亮和不亮的区别0-1，rect方框
...
class SSD1306_SPI(SSD1306):
  ...
  def write_data(self, buf):
    ...
    if currentBoard=="esp8266" or currentBoard=="esp32":
      ...
      self.spi.init(mode = pyb.SPI.MASTER,baudrate=self.rate, polarity=0, phase=0)   #这里为什么要用pyb.SPI.MASTER?这个值是260。machine.SPI中默认没有这个参数
    self.cs.high()
    self.dc.high()
    self.cs.low()
    global currentBoard
